...
import ChangeOutletConfirm from '@/components/ChangeOutletConfirm.vue'
...
const isOutletConfirmModalOpen = ref(false)
const selectedOutletId = ref(localStorage.getItem('active_outlet_id') || '')
const pendingOutletId = ref('')
...
const handleOutletChange = () => {
  // Jika keranjang ada isinya, tampilkan modal konfirmasi ganti outlet
  if (cartStore.items.length > 0) {
    isOutletConfirmModalOpen.value = true
    return
  }

  // Jika keranjang kosong, langsung arahkan ke katalog/halaman pemilihan outlet
  router.push({ name: 'catalog' })
}

const cancelOutletChange = () => {
  isOutletConfirmModalOpen.value = false
  pendingOutletId.value = ''
}

const executeOutletChange = () => {
  cartStore.clearCart() // Kosongkan keranjang belanja
  isOutletConfirmModalOpen.value = false
  router.push({ name: 'catalog' })
}
...
onMounted(async () => {
  try {
    ...
    // Ambil Data Outlet untuk Info Pengambilan
    const outletRes = await axios.get('/outlets')
    const outlets = outletRes.data?.data || outletRes.data || []
    const savedOutletId = localStorage.getItem('active_outlet_id')

    if (savedOutletId) {
      activeOutlet.value = outlets.find((o) => o.id === savedOutletId)
    } else if (outlets.length > 0) {
      activeOutlet.value = outlets[0]
    }

    // Ambil profil poin & daftar voucher
    ...
    const voucherRes = await axios.get('/voucher/index')
...
const submitCheckout = async () => {
  ...
  if (!activeOutlet.value) {
    errorMessage.value = 'Silakan pilih lokasi pengambilan (outlet) terlebih dahulu!'
    return
  }
  ...
  try {
    ...
    let scheduleInfo =
      scheduleType.value === 'now'
        ? '[Pesan Sekarang]'
        : `[Terjadwal: ${scheduledDate.value} ${scheduledTime.value}]`

    const payload = {
      outlet_id: activeOutlet.value.id, // Menyertakan outlet_id saat checkout
      ...
      amount_paid: finalPrice.value, // Diperbaiki: amoint_paid -> amount_paid
    }
...
<template>
  <div
    class="max-w-md mx-auto bg-gray-50 dark:bg-gray-950 h-dvh flex flex-col relative overflow-hidden transition-colors duration-300"
  >
    <!-- KONTROL UTAMA -->
    <main
      v-if="cartStore.items.length === 0"
      class="flex-1 overflow-y-auto p-4 flex flex-col items-center justify-center text-center"
    >
      <h3 class="text-xs font-bold text-gray-700 capitalize">Keranjang kosong!</h3>
      <button
        @click="router.push({ name: 'catalog' })"
        class="mt-4 bg-orange-500 text-white font-bold text-xs px-6 py-3 rounded-2xl shadow-md"
      >
        Pilih Menu
      </button>
    </main>

    <main v-else class="flex-1 overflow-y-auto p-4 space-y-4 pb-36 scroll-smooth">
      <div
        v-if="errorMessage"
        class="p-3 bg-red-50 text-red-600 text-xs rounded-xl text-center font-medium"
      >
        {{ errorMessage }}
      </div>

      <!-- LOKASI PENGAMBILAN (OUTLET) -->
      <div
        class="bg-white dark:bg-gray-900 p-3.5 rounded-2xl border border-gray-100 dark:border-gray-800 flex items-center justify-between shadow-sm"
      >
        <div class="flex items-start gap-3 min-w-0 pr-3">
          <div
            class="w-8 h-8 rounded-full bg-orange-50 dark:bg-orange-500/10 text-orange-500 flex items-center justify-center shrink-0 mt-0.5 text-sm"
          >
            📍
          </div>
          <div class="min-w-0">
            <p
              class="text-[9px] font-extrabold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-0.5"
            >
              Lokasi Pengambilan
            </p>
            <h4 class="text-xs font-bold text-gray-900 dark:text-white truncate">
              {{ activeOutlet ? activeOutlet.name : 'Memuat lokasi...' }}
            </h4>
            <p
              v-if="activeOutlet && activeOutlet.location"
              class="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5 truncate"
            >
        ...
        /> -->
        <button
          @click="handleOutletChange"
          class="text-[10px] font-bold text-orange-600 bg-orange-50 hover:bg-orange-100 dark:bg-orange-500/10 dark:hover:bg-orange-500/20 px-3.5 py-2 rounded-xl transition shrink-0 border border-orange-100 dark:border-orange-500/20"
        >
          Ubah
        </button>
      </div>
      ...
      <!-- Waktu Penyiapan -->
      <div
        class="bg-white dark:bg-gray-900 p-3.5 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-3 shadow-sm"
      >
        ...
        <div
          ...
          class="grid grid-cols-2 gap-2 pt-2 border-t border-gray-100 dark:border-gray-800"
        >
          <div>
          ...
          <div>
            <label class="text-[10px] text-gray-400 font-semibold block mb-1"
              >Jam (17:30 - 23:30)</label
            >
            <select
      ...
      <!-- SHOPEE STYLE: PILIH VOUCHER -->
      <div
        @click="isVoucherModalOpen = true"
        class="bg-white dark:bg-gray-900 p-3.5 rounded-2xl border border-gray-100 dark:border-gray-800 flex items-center justify-between cursor-pointer shadow-sm hover:border-orange-400 transition"
      >
        <div>
          ...
          <p class="text-[11px] text-gray-400">
            {{
              appliedVoucher
                ? `${appliedVoucher.name}${appliedVoucher.type === 'percentage' ? ` (${appliedVoucher.value}%)` : ''}`
                : 'Pilih atau masukkan voucher'
            }}
          </p>
        </div>
        <div class="flex items-center gap-1">
          <span v-if="appliedVoucher" class="text-xs font-bold text-amber-600">
      ...
      >
        <div class="flex items-center gap-2.5">
          <div>
            <p class="text-xs mb-1 font-bold text-gray-900 dark:text-white">
              Tukarkan
              {{ Number(customerPointsData.total_points || 0).toLocaleString('id-ID') }} Poin Wisuda
            </p>
            <p class="text-[10px] text-gray-400">
              <span v-if="hasEnoughPoints">Gunakan semua poin untuk potongan harga</span>
              <span v-else class="text-amber-600 dark:text-amber-400 font-small"
                >Minimal penukaran 1.000 poin</span
              >
            </p>
          </div>
        </div>
        ...
        <label
          ...
          :class="{ 'opacity-40 cursor-not-allowed': !hasEnoughPoints }"
        >
          <input
            type="checkbox"
      ...
      <div
        class="bg-white dark:bg-gray-900 p-3.5 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-3 shadow-sm"
      >
        <h3 class="text-xs font-bold text-gray-700 dark:text-white capitalize pb-1">
          Ringkasan Pesanan
        </h3>
        <div
          v-for="item in cartStore.items"
          :key="item.id"
          class="flex justify-between items-center text-xs py-1 border-b border-gray-50 dark:border-gray-800 last:border-none"
        >
          ...
          <div class="flex items-center justify-between gap-2 pt-1">
            <div
              class="flex items-center bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-2xs flex-shrink-0"
            >
              <button
                @click="confirmDecreaseQty(item)"
                class="px-2.5 py-1 text-xs font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"
              >
                -
              </button>
              <span class="px-2.5 text-xs font-bold text-gray-800 dark:text-gray-100">{{
                item.quantity
              }}</span>
              <button
                @click="item.quantity++"
      ...
      <!-- Rincian Pembayaran -->
      <div
        class="bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-2 text-xs shadow-sm"
      >
        <h3 class="text-xs font-bold text-gray-700 dark:text-white capitalize pb-1">
          Rincian Pembayaran
        </h3>
        ...
        <div
          class="flex justify-between items-center pt-2.5 border-t border-gray-100 dark:border-gray-800 text-xs font-medium text-gray-900 dark:text-white"
        >
          <span>Total Pembayaran</span>
          <span class="font-mono text-gray-700 dark:text-gray-300 text-base"
            >Rp {{ formatPrice(finalPrice) }}</span
          >
        </div>
    ...
    <div
      v-if="cartStore.items.length > 0"
      class="w-full fixed bottom-0 left-0 right-0 p-3 max-w-md mx-auto z-40 shadow-2xl"
    >
      <button
        @click="submitCheckout"
        :disabled="loading"
        class="w-full bg-orange-500 hover:bg-orange-600 text-white rounded-2xl px-5 py-3 shadow-md transition cursor-pointer disabled:opacity-50 flex items-center justify-between"
      >
        <div class="flex flex-col items-start min-w-0 pr-2">
          <span class="text-[9px] uppercase font-bold text-orange-200 tracking-wider"
            >Total Pembayaran</span
          >
          <span class="font-black text-sm font-mono tracking-tight truncate text-white"
            >Rp {{ formatPrice(finalPrice) }}</span
          >
        </div>
        <div
          class="flex items-center gap-1 font-black text-xs shrink-0 bg-orange-700/40 px-3.5 py-2 rounded-xl text-white"
        >
    ...
    >
      <div
        class="bg-white dark:bg-gray-900 w-full max-w-md rounded-t-3xl sm:rounded-2xl max-h-[80vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-200"
      >
        <div
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-2.5">
          <div
            v-if="appliedVoucher"
            class="p-3 bg-red-50 rounded-xl flex justify-between items-center text-xs"
          >
            <span class="font-bold text-red-600">Lepas Voucher Terpakai</span>
            <button
              @click="handleRemoveVoucher"
              class="px-3 py-1 bg-red-500 text-white rounded-lg font-bold"
            >
              Batal Pakai
            </button>
          </div>
          ...
          <div
            ...
            class="p-3.5 bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 rounded-2xl flex justify-between items-center cursor-pointer hover:border-orange-500 transition"
          >
            <div>
              <h4 class="text-xs font-bold text-gray-700 dark:text-white capitalize pb-1">
                {{ v.name }}
              </h4>
              <p class="text-[11px] text-amber-600 font-medium">
                <template v-if="v.type === 'percentage'">
                  Diskon {{ v.value }}%<span v-if="v.max_discount">
                    (Maks Rp {{ formatPrice(v.max_discount) }})</span
                  >
                </template>
                <template v-else> Potongan Rp {{ formatPrice(v.value) }} </template>
              </p>
            </div>
  ...
  <ChangeOutletConfirm
    :show="isOutletConfirmModalOpen"
    @cancel="cancelOutletChange"
    @confirm="executeOutletChange"
  />
  ...
  >
    <div
      class="bg-white dark:bg-gray-900 w-full max-w-xs rounded-3xl p-6 text-center shadow-2xl border border-gray-100 dark:border-gray-800 space-y-4 animate-in zoom-in-95 duration-200"
    >
      <div
        class="w-14 h-14 mx-auto rounded-full bg-red-100 dark:bg-red-500/20 flex items-center justify-center text-2xl text-red-600 dark:text-red-400"
      >
        🗑️
      </div>
      <div>
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed">
          ...
          <span class="font-bold text-gray-800 dark:text-gray-200"
            >"{{ itemPendingDelete?.name }}"</span
          >
          akan dihapus karena jumlahnya mencapai 0.
        </p>
      </div>
