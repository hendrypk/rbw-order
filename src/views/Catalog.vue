...
<template>
    <div class="max-w-md mx-auto bg-gray-50/50 min-h-screen relative pb-32">
        
        <header class="sticky top-0 bg-white/80 backdrop-blur-md z-40 border-b border-gray-100/80 transition-all">
            <div class="px-4 py-3 flex items-center justify-between">
                <div class="flex flex-col items-start">
                    <span class="text-[10px] font-bold uppercase tracking-widest text-orange-500">Official Store</span>
                    <h1 class="text-xs font-black text-gray-900 tracking-tight">Roti Bakar Wisuda</h1>
                </div>

                <div class="flex items-center gap-1.5 text-gray-700">
                    <button @click="toggleSearch" class="w-9 h-9 rounded-full hover:bg-gray-100 flex items-center justify-center transition text-gray-700 active:scale-95">
                    ...
                    <div class="relative inline-block text-left">
                        <button 
                            ...
                            class="w-9 h-9 rounded-full bg-orange-50 hover:bg-orange-100 text-orange-600 flex items-center justify-center transition border border-orange-100 active:scale-95"
                        >
                        ...
                        <div 
                            v-if="isDropdownOpen" 
                            class="absolute right-0 mt-2.5 w-48 bg-white/95 backdrop-blur-md border border-gray-100 rounded-2xl shadow-xl py-2 z-50 text-xs font-medium text-gray-700 divide-y divide-gray-100/60 animate-in fade-in zoom-in-95 duration-150"
                        >
                            <div class="px-1.5 pb-1">
                                <button 
                                    @click="goToProfile" 
                                    class="w-full text-left px-3 py-2 rounded-xl hover:bg-orange-50 hover:text-orange-600 flex items-center gap-2 text-gray-800 transition font-semibold"
                                >
                                    ✨ Profil Saya
                                </button>
                            </div>
                            <div class="px-1.5 pt-1">
                                <button 
                                    @click="handleLogout" 
                                    class="w-full text-left px-3 py-2 rounded-xl hover:bg-red-50 text-red-600 flex items-center gap-2 transition font-semibold"
                                >
                                    🚪 Keluar
                                </button>
                            </div>
            ...
            <div class="flex overflow-x-auto px-4 py-2.5 gap-2 no-scrollbar border-t border-gray-100/60 bg-white/50">
                <button 
        ...
        <div v-if="isSearchOpen" class="bg-white px-4 py-2.5 border-b border-gray-100 shadow-sm transition-all">
            <div class="relative flex items-center">
                <input 
                    ...
                    class="w-full bg-gray-100/80 text-xs font-medium text-gray-800 px-4 py-2.5 pl-10 pr-9 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500/50 transition"
                />
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 absolute left-3.5 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                
                <button @click="closeSearch" class="absolute right-3 text-gray-400 hover:text-gray-600 text-xs font-bold p-1">
                    ✕
                </button>
            </div>
        </div>
        ...
        <main class="p-4 space-y-6">
            <div v-if="loading" class="text-center py-12 text-gray-400 text-xs font-medium">Lagi nyiapin menu terenak buat kamu... 🍞</div>
            <div v-else-if="errorMessage" class="bg-red-50 text-red-600 p-4 rounded-2xl text-xs text-center font-medium">{{ errorMessage }}</div>
            <div v-else-if="products.length === 0" class="text-center py-12 text-gray-400 text-xs font-medium">Belum ada menu tersedia nih.</div>

            <div v-for="(items, categoryName) in groupedProducts" :key="categoryName" class="space-y-3">
                <div class="flex items-center justify-between">
                    <h2 class="text-sm font-black text-gray-900 tracking-tight flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-orange-500"></span>
                        {{ categoryName }}
                    </h2>
                    <span class="text-[10px] font-bold text-gray-400 bg-gray-200/50 px-2 py-0.5 rounded-full">{{ items.length }} Pilihan</span>
                </div>

                <div v-for="product in items" :key="product.id" class="bg-white rounded-2xl p-3 shadow-sm hover:shadow-md transition border border-gray-100 flex gap-3.5 relative items-center group">
                    
                    <div class="w-20 h-20 bg-gray-100 rounded-2xl overflow-hidden flex-shrink-0 relative">
                        <img 
                    ...
                    <div class="flex-1 min-w-0 flex flex-col justify-between h-full py-0.5">
                        ...
                        <div class="flex items-center justify-between mt-2.5">
                            <span class="font-black text-gray-900 text-xs">Rp {{ formatPrice(product.price) }}</span>

                            <div>
                                <button 
                                    v-if="getCartItemQuantity(product) === 0" @click="handleAdd(product)"
                                    class="bg-orange-50 hover:bg-orange-500 hover:text-white text-orange-600 border border-orange-200/60 px-4 py-1.5 rounded-xl text-xs font-bold transition-all duration-200 shadow-sm active:scale-95">
                                    + Add
                                </button>
        ...
        <div v-if="cartStore.totalItems > 0" class="fixed bottom-4 left-4 right-4 max-w-md mx-auto z-40 animate-in slide-in-from-bottom-5 duration-300">
            <router-link to="/checkout" class="bg-gradient-to-r from-gray-900 to-gray-800 hover:from-black hover:to-gray-900 w-full rounded-2xl px-4 py-3.5 flex justify-between items-center shadow-xl shadow-black/15 active:scale-[0.98] transition border border-white/10">
